<template>
  <div class="bg-base-200 px-4 py-3 flex items-center justify-between border-b border-base-300">
    <div class="flex items-center gap-2">
      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 5a1 1 0 011-1h4a1 1 0 011 1v7a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM14 5a1 1 0 011-1h4a1 1 0 011 1v7a1 1 0 01-1 1h-4a1 1 0 01-1-1V5zM4 16a1 1 0 011-1h4a1 1 0 011 1v3a1 1 0 01-1 1H5a1 1 0 01-1-1v-3zM14 16a1 1 0 011-1h4a1 1 0 011 1v3a1 1 0 01-1 1h-4a1 1 0 01-1-1v-3z" />
      </svg>
      <span class="font-semibold text-sm text-base-content">PlantUML Diagram</span>
    </div>
    <div class="flex items-center gap-1">
      <button 
        @click="$emit('theme')" 
        class="btn btn-ghost btn-xs btn-square"
        title="Theme settings"
      >
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-base-content" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zm0 0h12a2 2 0 002-2v-4a2 2 0 00-2-2h-2.343M11 7.343l1.657-1.657a2 2 0 012.828 0l2.829 2.829a2 2 0 010 2.828l-8.486 8.485M7 17h.01" />
        </svg>
      </button>
      <button 
        @click="$emit('copy')" 
        class="btn btn-ghost btn-xs btn-square"
        :class="{ 'btn-success': copied }"
        title="Copy code"
      >
        <svg v-if="!copied" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-base-content" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
        </svg>
        <svg v-else xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-base-content" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
        </svg>
      </button>
      <button 
        @click="$emit('download')" 
        class="btn btn-ghost btn-xs btn-square"
        title="Download image"
      >
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-base-content" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
        </svg>
      </button>
      <button 
        @click="$emit('copy-image')" 
        class="btn btn-ghost btn-xs btn-square"
        :class="{ 'btn-success': imageCopied }"
        title="Copy image"
      >
        <svg v-if="!imageCopied" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-base-content" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
        </svg>
        <svg v-else xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-base-content" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
        </svg>
      </button>
      <button 
        @click="$emit('toggle-code')" 
        class="btn btn-ghost btn-xs btn-square"
        title="Toggle code"
      >
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-base-content" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" />
        </svg>
      </button>
    </div>
  </div>
</template>

<script setup lang="ts">
interface Props {
  copied?: boolean
  imageCopied?: boolean
}

withDefaults(defineProps<Props>(), {
  copied: false,
  imageCopied: false
})

defineEmits<{
  theme: []
  copy: []
  download: []
  'copy-image': []
  'toggle-code': []
}>()
</script>